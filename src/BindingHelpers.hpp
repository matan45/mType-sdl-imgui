#pragma once
/*
 * Helpers shared by SdlBindings.cpp and ImGuiBindings.cpp. Each .cpp picks
 * its own exception-type string ("SdlError" / "ImGuiError") via the `kEx`
 * constant in its anonymous namespace, so call sites stay terse:
 *
 *   if (!requireArgs(ctx, argc, 3, "__native__foo", kEx)) return ...;
 *   SDL_Window* w = findOrRaise(g_windows, g_host->getInt(args[0]),
